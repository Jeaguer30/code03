...
class Controller_formulario extends CI_Controller
{
    ...
    public function index(){
        ...
        $_SESSION['_SESSION_CALL_DATE'] = $_GET['SQLdate'];
        ...
        if($_SESSION['_SESSION_UNIQUE_ID'] != $id_unico && $_SESSION['_SESSION_UNIQUE_ID'] != "" && $_SESSION['_SESSION_UNIQUE_ID'] != 0){
            ...
            //$id_call  = $this->Modelo_formulario->insert_call('cvc_call', array('uniqueid' => $_SESSION['_SESSION_UNIQUE_ID'],  'f_add' => date('Y-m-d H:i:s'), 'user_add'  => $_SESSION['_SESSIONUSER'], 'user_marcador'  => $_SESSION['_SESSION_USER_ID'], 'celular'  => $_SESSION['_SESSION_PHONE_NUMBER'], 'fecha_marcador'  => $_SESSION['_SESSION_CALL_DATE']  ));
              
            $id_call  = $this->Modelo_formulario->insert_call('cvc_call', array('uniqueid' => $_SESSION['_SESSION_UNIQUE_ID'],  'f_add' => date('Y-m-d H:i:s'), 'user_add'  => $_SESSION['_SESSIONUSER'], 'user_marcador'  => $_SESSION['_SESSION_USER_ID'], 'celular'  => $_SESSION['_SESSION_PHONE_NUMBER'], 'fecha_marcador'  => $_SESSION['_SESSION_CALL_DATE'] , 'lead_id'  => $_SESSION['_SESSION_LEAD_ID']  ));
        ...
        $header['id_menu']  = $this->Mdl_compartido->retornarcampo('seg_formulario','ruta','config_sub_menu','id_menu');
        $id_sub_menu        = $this->Mdl_compartido->retornarcampo('seg_formulario','ruta','config_sub_menu','id');
    

        //Validamos que tenga el permiso
        $permiso = $this->Mdl_compartido->permisos_menu_mejorado('seg_formulario',$tipo_user);
        if (!$permiso){
            redirect('');
        }
    ...
    function valida_uniqueid(){
        ...
        if($uniqueid==''){        
            $uniqueid =   $this->Modelo_formulario->obtener_uniqueid($_SESSION['_SESSION_USER_ID'],$_SESSION['_SESSION_PHONE_NUMBER']);
            //Armamos link nuevo
            $uniqueid = 'https://geochasqui.com/sistema_cvc/seg_formulario?user='.$_SESSION['_SESSION_USER_ID'].'&phone_number='.$_SESSION['_SESSION_PHONE_NUMBER'].'&uniqueid='.$uniqueid;
        }else{
            $uniqueid='ok';
        }
    ...
    function actualizar(){     
        ...
        $result = $this->Modelo_formulario->update('cvc_solicitudes',$array,$id,'id');    
        echo $result;
    }
    ...
    function get_registros(){
        ...
        $result_pag = $this->Modelo_formulario->get_total_registros_pag('cvc_solicitudes a');
        ...
        $result     = $this->Modelo_formulario->get_registros('cvc_solicitudes a',$limite,$paginacion);

        $r_estado_solicitud = $this->Modelo_formulario->get_select('cvc_estado_solicitud');
    ...
    function insert(){
        ...
        $result = $this->Modelo_formulario->insert('cvc_solicitudes',$array);
        echo $result;
    }
       ...
       // $mot       = trim($this->input->post('mot',true));
        ...
        switch ($tipo) {
            case 'contacto':
                ...
                $result = $this->Modelo_formulario->insert('cvc_contactos',$array);
                break;
            case 'seguimiento':
                $array = array(
                    'descripcion'      => $des,
                ...
                $result = $this->Modelo_formulario->insert('cvc_seguimientos',$array);
                break;                
            default:
                break;
        }
    ...
    function get_datos_select_x_id(){
        ...
        $result    = $this->Modelo_formulario->get_select_x_id($table,$id_seleccion,'id_area');
    ...
    function get_registros_adicional(){
        ...
        switch ($tipo) {
            case 'seguimiento':
                ...
                $result    = $this->Modelo_formulario->get_select_x_id_join($table,$id_sol,'id_solicitud',$table2,$table3,$table4,$table5 );
                break;
            case 'contacto':
                $table='cvc_contactos';
                $result    = $this->Modelo_formulario->get_select_x_id($table,$id_sol,'id_solicitud');
                break;
            default:
                break;
        }
            ...
            foreach ($result as $key) {
                switch ($tipo) {
                    case 'seguimiento':
                        ...
                        $r_motivo = $this->Modelo_formulario->get_select('cvc_call_motivo');
    ...
    function eliminar(){
        ...
        switch ($tipo) {
            case 'seguimiento':
                ...
                $result    = $this->Modelo_formulario->eliminar($table,$id_reg);
                break;
            case 'contacto':
                $table='cvc_contactos';
                $result    = $this->Modelo_formulario->eliminar($table,$id_reg);
            ...
            case 'solicitud':
                ...
                $result = $this->Modelo_formulario->update('cvc_solicitudes',$array,$id_sol,'id');
                break;                            
            default:
                break;
        }
    ...
    public function subir_foto(){
        ...
        if ($this->upload->do_upload('file')){
            ...
            $res = $this->Modelo_formulario->update('tb_usuarios',$array,$id);
            if($res){
                    $valida='si';
            }else{
                    $valida='no';
    ...
    */
    function get_indicadores(){
        $r_indicadores = $this->Modelo_formulario->get_totales_indicador('cvc_solicitudes','cvc_estado_solicitud');
        $activo     =0;
        $en_espera  =0;
        $completado =0;
        $cancelado  =0;
    ...
    */
    function get_select_filtros(){
        $r_estado_solicitud = $this->Modelo_formulario->get_select('cvc_estado_solicitud');
        //$r_tipo_contrato    = $this->Modelo_formulario->get_select('tipo_contrato');
        //$r_tipo_frecuencia  = $this->Modelo_formulario->get_select('tipo_frecuencia');
        $r_solicitud  = $this->Modelo_formulario->get_filtro_mes('cvc_solicitudes');
    ...
    */
    ...
    function activar_filtro_seguimiento() {    
        ...
        if (empty($dato)) {
            ...
            // Si $dato es un array, guarda cada valor en la sesión
            if (is_array($dato)) { //dato es un array
                $_SESSION['SESSION_ESTADO_SEGUIMIENTO'] = $dato;
            } else { //lo convierte en una ray
                $_SESSION['SESSION_ESTADO_SEGUIMIENTO'] = [$dato];
            }
        }
    }
    ...
    */
    ...
    function buscar_registro(){
        ...
        $result    = $this->Modelo_formulario->get_registros_condicion($table,$dato);
    ...
    */
    ...
    function validar_solicitud_seguimiento() {
        ...
        $count = $this->Modelo_formulario->validar_seguimiento('cvc_solicitudes', $id2);
    
        echo json_encode($count);
    }

    function add_solicitud(){
        $id       = trim($this->input->post('id',true)); //iscom
        $nom      = trim($this->input->post('nom',true)); //nombre
        $tip       = trim($this->input->post('tip',true));  //1regular 2libre 3no reconoce


        //BUSCAMO AL CLIENTE CON EL ISCOM EL LA TABLA DE CLIENTE 
        $dato_iscom_cliente =     $this->Mdl_compartido->retornarcampo( $id,'codigo_iscom','cvc_clientes_libres','codigo_iscom'); 

        //SI EL CLEINTE NO EXISTE
        if($dato_iscom_cliente !=  $id  ){
            ...
            $id_regular = $this->Modelo_formulario->insert_call('cvc_clientes_libres',$array_cliente);

        }else{ //SI EXISTE 
            $dato_isco_cliente =     $this->Mdl_compartido->retornarcampo( $id,'codigo_iscom','cvc_clientes_libres','id'); 
            $id_regular = $dato_isco_cliente;
        }
        ...
        //ACTUALIZO EL CLIENTE EN LA TABLA CALL

        $array_call1 = array(
            'codigo_cliente'          =>$id_regular                       
        );
        $result = $this->Modelo_formulario->update('cvc_call',$array_call1,$_SESSION['_SESSION_CALL_ID'],'id');
        ...
        //VALIDAMOS SI EXISTE EN SOLICITUDE LA LLAMADA
        $dato_call =     $this->Mdl_compartido->retornarcampo( $_SESSION['_SESSION_CALL_ID'],'id_call','cvc_solicitudes','id_call'); 

        //SI NO EXITE PODEMOS LA LAMADA IONSERTAMOS
        if($_SESSION['_SESSION_CALL_ID'] != $dato_call ){
          
            $array_insert = array(            
                'id_call'             => $_SESSION['_SESSION_CALL_ID'], 
                'id_cliente'          =>$id_regular,    
                'id_estado_solicitud' =>2,           
                'f_registro'          => date('Y-m-d H:i:s'),
                'id_usuario'          => $_SESSION['_SESSIONUSER']
            );
            $result = $this->Modelo_formulario->insert('cvc_solicitudes',$array_insert);
            ...
            //$result = $this->Modelo_formulario->insert('cvc_solicitudes',$array_upd);
            $result = $this->Modelo_formulario->update('cvc_solicitudes',$array_upd,$_SESSION['_SESSION_CALL_ID'],'id_call');
        }
    ...
    function get_datos_call(){
        ...
        //$result    = $this->Modelo_formulario->get_data_x_uniq('cvc_call',$id);
        $result    = $this->Modelo_formulario->get_data_x_uniq('cvc_call',$_SESSION['_SESSION_UNIQUE_ID']);
        if($result!='error'){
            foreach ($result as $key) { 
                ...
                $ar['id_localidad']     =$key->id_localidad;
    ...
    function action_uniqueid_call(){
        ...
        $loc        = trim($this->input->post('loc',true));
        $tipo       = trim($this->input->post('tipo',true));
        ...
        if($id_unique== $unid){//validamos para ver si insert o upd
            ...
            $result = $this->Modelo_formulario->update('cvc_call',$array_upd,$unid,'uniqueid');
            ...
            $result = $this->Modelo_formulario->insert('cvc_call',$array_insert);
        }
    ...
    function load_motivo_call(){
        ...
        $resultado = $this->Modelo_formulario->get_data_load('cvc_call_motivo');
    ...
    function load_localidad_call(){
        $id       = trim($this->input->post('id',true));

        $resultado = $this->Modelo_formulario->get_data_load('cvc_call_localidad');
        $cadena='<option value=""> Localidad</option>'; 
        if($resultado!='error'){        
            foreach($resultado as $key){       
                if($id==$key->id){
                    $cadena .= '<option selected value="'.$key->id.'">'.$key->descripcion.'</option>';
    ...
    function load_emergencia(){
        ...
        $resultado = $this->Modelo_formulario->get_data_load_contacto_emergencia('cvc_contactos_area_cvc');
    ...
    function load_distribuidores_cvc(){
        ...
        $resultado = $this->Modelo_formulario->get_data_load_contacto_distribuidor('cvc_contactos_distribuidores');
    ...
    function get_registros_call(){
       ...
       // $id_cliente       = trim($this->input->post('id',true));
        ...
        //Obtenemos los datos para el select
        $result     = $this->Modelo_formulario->get_registros_call('cvc_solicitudes a',$limite,$paginacion);
    ...
    function upd_registros(){
        ...
        switch ($tipo) {
            case 'upd_seguimiento':
                ...
                //$result = $this->Modelo_formulario->insert('cvc_seguimientos',$array);
                $result = $this->Modelo_formulario->update('cvc_seguimientos',$array,$id,'id');
                break;                
            default:
                break;
        }
    ...
    public function get_distribuidores() {
        ...
        $result = $this->Modelo_formulario->get_distribuidores();
    ...
    public function get_contactos_area_cvc() {
        ...
        $result = $this->Modelo_formulario->get_contactos_area_cvc();
        $cadena = '';
    
        if ($result != 'error') {
            // Obtener los grupos de contacto y sus cantidades
            $result2 = $this->Modelo_formulario->grupos_contacto_emergencia();
            $grupos = [];
    ...
    public function get_distribuidores_cvc() {
        // Obtener los contactos utilizando el modelo
        $result = $this->Modelo_formulario->get_solicitudes_cvc();
        $cadena = '';
    
        if ($result != 'error') {
            // Obtener los grupos de contacto y sus cantidades
            $result2 = $this->Modelo_formulario->grupos_contacto_solicitud();
            $grupos = [];
            foreach ($result2 as $key2) {
    ...
    public function get_correos_distribuidores_cvc() {
        // Obtener los contactos utilizando el modelo
        $result = $this->Modelo_formulario->get_correos_distribuidor_cvc();
        $cadena = '';
    
        if ($result != 'error') {
            // Obtener los grupos de contacto y sus cantidades
            $result2 = $this->Modelo_formulario->grupos_correos_distribuidor_cvc();
            $grupos = [];
            foreach ($result2 as $key2) {
                $grupos[$key2->descripcion] = $key2->cantidad_contactos;
            }
    ...
    public function get_clientes() {
        ...
        $result = $this->Modelo_formulario->get_clientes();
        ...
        if ($result && $result !== 'error') {
            foreach ($result as $key) {
                ...
                if ($key->id_tipo_cliente == 1) {
                    $tipo_cliente = 'Cliente Regular';
                    $icon_cliente = 'bx bxs-user';
                } elseif ($key->id_tipo_cliente == 3) {
                    $tipo_cliente = 'Cliente No Reconocido';
                    $icon_cliente = 'bx bxs-pen';
                } else {
                    $tipo_cliente = 'Cliente Libre';
                    $icon_cliente = 'bx bxs-bank';
                }
                    ...
                    <a class="list-group-item list-group-item">
                        <div class="d-flex align-items-center">
                            <div class="avatar-sm flex-shrink-0 me-3">
                                <div class="avatar-title bg-soft-dark text-white rounded-circle font-size-24">
                                    <i class="'.$icon_cliente.'"></i>
                                </div>
                            </div>
                            ...
                            <div class="flex-grow-1 me-2">
                                <div>
                                    ...
                                    <p class="font-size-13 text-muted mb-0"><b>Ruc: </b> ' . $key->ruc . '</p>
                                    <p class="font-size-13 text-muted mb-0"> <b>Codigo: </b>  <mark>' . $key->codigo_iscom . '</mark></p>
                                </div>
                            </div>';

                            if($key->id_tipo_cliente == 2 ){
                                $cadena .= '
                                <div class="flex-grow-1">
                                    <div>                           
                                        ...
                                        <p class="font-size-13 text-muted mb-0"> <b>Dir. Suministro: </b> ' . $key->direccion_suministro . '</p>
    ...
    public function get_solicitudes() {
        ...
        // Obtener las solicitudes utilizando el modelo
        $result = $this->Modelo_formulario->get_solicitudes();
        ...
        if ($result != 'error') {
            foreach ($result as $key) {
                ...
                ($key->id_estado_solicitud == 5 ? 'bg-danger' :
                ($key->id_estado_solicitud == 4 ? 'bg-danger' :
                ($key->id_estado_solicitud == 6 ? 'bg-success' : 'bg-light'))));


                $text = $key->id_estado_solicitud == 2 ? 'text-warning' :
                ($key->id_estado_solicitud == 3 ? 'text-info' :
                ($key->id_estado_solicitud == 5 ? 'text-danger' :
                ($key->id_estado_solicitud == 4 ? 'text-danger' :
                ($key->id_estado_solicitud == 6 ? 'text-success' : 'bg-light'))));
                    ...
                    <li class="activity-list activity-border">
                        ...
                        <div class="timeline-list-item">
                            <div class="d-flex">
                                ...
                                <div class="flex-grow-1">
                                    <div>           
                                        <p class="font-size-13 text-muted mb-0"> <b>Estado: </b>  <b class="' . $text . '" >' . $key->estado_solicitud . '  </b> </p>          
    ...
    public function get_distribuidores_telefonos() {
        ...
        $result = $this->Modelo_formulario->get_distribuidores_telefonos();
        $cadena = '';
        if ($result && $result !== 'error') {
            foreach ($result as $key) {
    ...
    public function get_distribuidores_correos() {
        ...
        $result = $this->Modelo_formulario->get_distribuidores_correos();
    ...
    function upd_motivo_llamada(){
        ...
        $result = $this->Modelo_formulario->update('cvc_call',$array,$id,'id');
