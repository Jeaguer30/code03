var tb_table_consulta; 
var id_cliente;
var id_visita;
var latitud;
var longitud;
var latitud_cliente;
var longitud_cliente;
let timer_2 = 0;
const delayValue_2 = 100 ;

   function ver_mdl(tipo){
        switch (tipo) {
        
            case 'mdl_detalle_resumen':              
                $('#mdl_detalle_marcador').modal('show');           
                break;
            case 'mdl_registrar_asignacion':
                $('#mdl_registrar_asignacion').modal('show');             
                break;
...
/* funcion para insertar una nueva asignacion */
    ...
    function insertar_asignacion(){
        ...
        $.ajax({
            type: "POST",
            url: 'https://geochasqui.com/sistema_geo/Controller_ruteo_prioridad_geo/insertar_asignacion',
            data: {
            ...
            success: function(data){                         
                ...
                $('#mdl_registrar_asignacion').modal('hide');
                limpiar_formulario();              
            }
        });       
...
/* funciones para la actualizacion de las coordenadas */

    function get_data_coordenadas(id){
        $('#ubicacion_cod_cliente').val(id);

        $.ajax({
            type: "POST",
            //dataType: 'json',
            url: 'https://geochasqui.com/sistema_geo/Controller_ruteo_prioridad_geo/get_data_coordenadas',
            data: {
                'id':id
            }, 
            beforeSend:function(){
            ...
            success: function(data){ 
                ...
                if(res.mensaje!='error'){
                    var json         = eval("(" + data + ")");
                    if(json.mensaje=='ok'){
                        //Colocamos todas las coordenadas
                        switch (json.seleccion) {
                            case "PDV":
                                $('#ubicacion_pdv').prop("checked",true);
                                break;
                            case "ACMOVIL":
                                $('#ubicacion_acmovil').prop("checked",true);
                                break;
                            case "WELLINGTON":
                                $('#ubicacion_wellington').prop("checked",true);
                                break;
                            case "OTROS":
                                $('#ubicacion_otros').prop("checked",true);
                                break;                                                                                       
                            default:
                                break;
                        }

                        $('#ubicacion_pdv').val(json.ubicacion_pdv);
                        $('#cor_pdv').text(" ( "+json.ubicacion_pdv+" ) ");
                        $('#ubicacion_acmovil').val(json.ubicacion_acmovil);
                        $('#cor_acmovil').text(" ( "+json.ubicacion_acmovil+" ) ");
                        $('#ubicacion_wellington').val(json.ubicacion_wellington);
                        $('#cor_wellington').text(" ( "+json.ubicacion_wellington+" ) ");
                        $('#ubicacion_otros_dato').val(json.ubicacion_otros);
                        $('#mdl_ubicacion_ruteo').modal('show');
                    }else{
                    }
                }else{
                } 
    ...
    function actualizar_coordenadas(){
        ...
        var tipo='';
        var dato='';
        if($('#ubicacion_pdv').prop('checked')){
            t++;
            tipo ='ubicacion_pdv';
            dato = $('#ubicacion_pdv').val();
        }

        if($('#ubicacion_acmovil').prop('checked')){
            t++;
            tipo='ubicacion_acmovil';
            dato = $('#ubicacion_acmovil').val();
        }

        if($('#ubicacion_wellington').prop('checked')){
            t++;
            tipo='ubicacion_wellington';
            dato = $('#ubicacion_wellington').val();

        }

        if($('#ubicacion_otros').prop('checked')){
            t++;
            tipo='ubicacion_otros';
            dato = $('#ubicacion_otros_dato').val();
            if(dato!=''){

            }else{
                alertify.error('Ingrese las coordenadas: latitud y longitud..');
                return;
            }
        }

        var codigo  = $('#ubicacion_cod_cliente').val();
        if(t==1){
            $.ajax({
                type: "POST",
                url: 'https://geochasqui.com/sistema_geo/Controller_ruteo_prioridad_geo/actualizar_coordenadas',
                data: {'tipo':tipo,'dato':dato,'codigo':codigo}, 
                beforeSend:function(){
                },   
                success: function(data){
                    if(data=='error_latitud'){
                        alertify.error('Ocurrió un error, ingrese la latitud coorrectamente...');
                    }else if(data=='error_longitud'){
                        alertify.error('Ocurrió un error, ingrese la longitud coorrectamente...');
                    }else if(data=='error_coordenadas'){
                        alertify.error('Ocurrió un error, las coordenadas ingresadas no corresponden en algún punto del mapa del perú...');
                    }else if(data=='sin_tipo'){
                        alertify.error('Ocurrió un error, no se seleccionó el tipo de coordenada...');
                    }else if(data=='sin_dato'){
                        alertify.error('Ocurrió un error, no contiene alguna coordenada la selección enviada...');
                    }else if(data=='ok'){
                        alertify.success('Coordenadas actualizadas, se actualizó correctamente las coordenadas seleccionadas...');
                        $('#mdl_ubicacion_ruteo').modal('hide');
                        //Para gestor mapa__
                        open_modal_menu_gestor_mapa();
                        /*Fin de gestión mapa*/
                    }else{
                        alertify.error('Hubo un error al actualizar las coordenadas, vuelva a intentarlo.');
                    }
                },
                error: function(){
            ...
            alertify.error('Seleccione una sola ubicación para el cambio.');
            return; 
        }
    }
...
/* funcion para actualizacion de la asignacion */

    function get_update_asignacion(id){
        $.ajax({
            type: "POST",
            //dataType: 'json',
            url: 'https://geochasqui.com/sistema_geo/Controller_ruteo_prioridad_geo/get_update_asignacion',
            data: {
                'id':id
            }, 
            beforeSend:function(){
            ...
            success: function(data){ 
                ...
                var res = jQuery.parseJSON(data);
                if(res.mensaje!='error'){
                    $('#txt_cliente').val(res.nom_cliente);                      
                    $('#txt_usuario').val(res.nom_user);
                    $('#txt_fecha_ruteo').val(res.fecha_ruteo);
                    $('#slc_reasignacion').val(res.dni);                           
                    $('#mdl_upd_asignacion').modal('show');
                    var json         = eval("(" + data + ")");
                    if(json.mensaje=='ok'){
                        $('#slc_reasignacion').val(json.orden); 
                        $('#botones_upd').html(json.botones);   
                    }else{
                    }
    ...
    function upd_prioridad(id){
        var value_select = $('#slc_reasignacion').val();

        if(value_select==''){
            alertify.error('Seleccione el tipo de marcador para actualizar');
            return; 
        }
       
        $.ajax({
            type: "POST",
            url: 'https://geochasqui.com/sistema_geo/Controller_ruteo_prioridad_geo/upd_prioridad',
            data: {'id':id,'select_a':value_select}, 
            beforeSend:function(){
            },   
            success: function(data){   
                alertify.success('ok');
                tb_table_ruteo.ajax.reload();
...
function search_cliente(id_cliente, map, marker) {
  
    id_cliente = id_cliente; // asignar id
    $.ajax({
        type: "POST",
        ...
        success: function (e) {
            ...
            if (res.mensaje != 'error') {
                ...
                get_data_detalle_campania(id_cliente);    
                get_data_historial_asignacion1(id_cliente);
                get_data_control_qa(id_visita);               
            } else {              
                var errorInfowindow = new google.maps.InfoWindow({
                    content: 'No cuenta con visitas'
                });
                errorInfowindow.setContent('No cuenta con visitas');
...
//--------------------- LOAD ---------------------------
    ...
    function load_zonas_upd(){
        ...
        if(datos!=''){
            ...
            type:'post',
                contentType:false,
                url:'https://geochasqui.com/sistema_geo/Controller_visita/load_zonas',
                data: data,
                processData:false,
                beforeSend:function(){
                },
...
//--------------------- GET TABLE ---------------------
    ...
    function get_data_programacion(){
        $.ajax({
            type: "POST",
            url: 'https://geochasqui.com/sistema_geo/Controller_visita/get_data_table_programacion',
            data: {
            //'id':id
            }, 
            beforeSend:function(){
    ...
    function get_data_almacenes(){
        $.ajax({
            type: "POST",
            url: 'https://geochasqui.com/sistema_geo/Controller_visita/get_data_table_almacenes',
            data: {
            //'id':id
            }, 
            beforeSend:function(){
    ...
    function get_data_transportistas(){
        $.ajax({
            type: "POST",
            url: 'https://geochasqui.com/sistema_geo/Controller_visita/get_data_table_transportistas',
            data: {
            //'id':id
            }, 
            beforeSend:function(){
    ...
    function get_data_contactos_comercial(){
        $.ajax({
            type: "POST",
            url: 'https://geochasqui.com/sistema_geo/Controller_visita/get_data_table_contactos_comercial',
            data: {
            //'id':id
            }, 
            beforeSend:function(){
    ...
    function eliminar_asignacion(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: "POST",
                    url: 'https://geochasqui.com/sistema_geo/Controller_visita/eliminar_asignacion',
                    data: {
                        'id': id
                    },
                    beforeSend: function () {
...
function get_list_events(id_cliente){               
    $.ajax({
        type: "POST",
        url: 'https://geochasqui.com/sistema_geo/C_mod_form_event/get_list_events1',
        data: {
            'id_cliente':id_cliente
        }, 
        beforeSend:function(){
...
function get_indicadores_eventos(id_cliente) {
    ...
    $.ajax({
        ...
        type: "POST",
        url: 'https://geochasqui.com/sistema_geo/C_mod_form_event/get_indicadores_eventos',
        data: {
...
function edit_control_event(id) {
    $.ajax({
        ...
        url: 'https://geochasqui.com/sistema_geo/C_mod_form_event/edit_control_event',
        data: {
            'id':id
        },
        beforeSend: function () {
...
function edit_event(id){          
    $.ajax({
        type: "POST",
        //dataType: 'json',
        url: 'https://geochasqui.com/sistema_geo/C_mod_form_event/edit_event',
        data: {
            'id':id
        }, 
        beforeSend:function(){
...
function delete_event(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                type: "POST",
                url: 'https://geochasqui.com/sistema_geo/C_mod_form_event/delete_event',
                data: {
                    'id': id
                },
                beforeSend: function () {
...
function add_control_event(){    
    ...
    $.ajax({
        type: "POST",
        url: 'https://geochasqui.com/sistema_geo/C_mod_form_event/add_control_event',
        data: {
            'txto_id1':txto_id1,
...
function refresh_event() {
    ...
    timer_2: 1100,
    timer_2ProgressBar: true,
        didOpen: () => {
            Swal.showLoading();
            const timer_2 = Swal.getPopup().querySelector("b");
            timer_2Interval = setInterval(() => {
            timer_2.textContent = `${Swal.gettimer_2Left()}`;
            }, 100);
        },
        willClose: () => {
            clearInterval(timer_2Interval);
        }
        }).then((result) => {
            /* Read more about handling dismissals below */
            if (result.dismiss === Swal.DismissReason.timer_2) {
                console.log("I was closed by the timer_2");
            }
    });
...
function abrir_control_qa(){                  
    $('#mdl_add_control_qa').modal('show'); 
}

function abrir_control_qa_editar(id){ 

    $.ajax({
        type: "POST",
        dataType: 'json',
        url: 'https://geochasqui.com/sistema_geo/Controller_visita/get_control_qa',
        data: {
            'id': id
        },
        beforeSend: function () {                      
        ...
        success: function (res) {
            if (res.mensaje !== 'error') {
                ...
                $('#txt_qa_id').val(res.id);
                $('#txt_qa_control_edit').val(res.control);
                $('#txt_qa_obs_edit').val(res.obs);                   
                $('#mdl_edit_control_qa').modal('show');//open    
...
function add_control_qa(){
              
    //var id_visita =7313;      
    var control =$('#txt_qa_control').val();
    var obs  =$('#txt_qa_obs').val();
   

    $.ajax({
        type: "POST",
        //dataType: 'json',
        url: 'https://geochasqui.com/sistema_geo/Controller_visita/add_control_qa',
        data: {
        ...
        success: function(data){ 
            alertify.success('Se agrego control de calidad.');
            $('#mdl_add_control_qa').modal('hide');
            get_data_control_qa(id_visita);  
            abrir_detalle_visita(id_cliente,id_visita);     
            $('#txt_qa_control').val('');
            $('#txt_qa_obs').val('');
...
function upd_control_qa(){
              
    //var id_visita =7313;    
    var id =$('#txt_qa_id').val();  
    var control =$('#txt_qa_control_edit').val();
    var obs  =$('#txt_qa_obs_edit').val();
    

    $.ajax({
        type: "POST",
        //dataType: 'json',
        url: 'https://geochasqui.com/sistema_geo/Controller_visita/upd_control_qa',
        data: {
        ...
        success: function(data){ 
            alertify.success('Se agrego control de calidad.');
            $('#mdl_edit_control_qa').modal('hide');
            get_data_control_qa(id_visita);  
...
function get_data_control_qa(id_visita){  
    //var id_visita1 = 37123;
    $.ajax({
        type: "POST",
        url: 'https://geochasqui.com/sistema_geo/Controller_visita/get_data_control_qa',
        data: {
        'id_visita':id_visita
        ...
        success: function(data){ 
            ...
            if(json != 'error'){
                for (let index = 0; index < json.length; index++) {
                    ...
                    contenido += '<td style="font-size:12px; text-align: center;">' + contador + '</td>';                          
                    //contenido += '<td style="font-size:12px; text-align: center;">' + json[index]['id'] + '</td>';   
                    //contenido += '<td style="font-size:12px; text-align: center;">' + json[index]['id_visita'] + '</td>';  
                    contenido += '<td style="font-size:12px; text-align: center;">' + json[index]['control_usuario'] + '</td>';  
                    contenido += '<td style="font-size:12px; text-align: center;">' + json[index]['control_usuario_qa'] + '</td>';     
                    contenido += '<td style="font-size:12px; text-align: center;">' + json[index]['fecha_registro'] + '</td>';    
                    contenido += '<td style="font-size:12px; text-align: center;">' + json[index]['control_qa'] + '</td>';                                                                                                             
                    //contenido += '<td style="font-size:12px; text-align: center;">' + json[index]['observación'] + '</td>'; 

                    contenido += '<td style="text-align: center;"><button class="btn btn-sm btn-secondary p-1" onclick="abrir_control_qa_editar(' + json[index]['id'] + ')"><i class="mdi mdi-pencil"></i></button></td>';                                                        
            ...
            $('#tb_observaciòn_qa').html(contenido);
        }
    });
}
/*FIN  */